						  ...
						  <template v-if="order_status==0"><!--判断哪个订单状态-->
							  <template v-for="item in demoData"><!--具体的数据结构写入  -->
									 <div  id="nopay-order" class="weui_tab_bd_item weui_tab_bd_item_active">
											 ...
											 <a v-on:click="jump_url(item.id,item.url) class="weui-media-box weui-media-box_appmsg clear">
													...
													<div class="weui-media-box__bd">
														...
														<h4 class="weui-media-box__title">￥<span id="singel-price">120.00{{item.title}}</span>&nbsp;&nbsp;
															<span id="singel-number">X1{{item.title}}</span></h4>
													</div>
												</a>
												<div class="order-foot float-right">共<span>1</span>件产品，合计<span>23234</span>元（含运费<span>10.00{{item.title}}</span>元）</div>
